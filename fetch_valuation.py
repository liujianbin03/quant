# -*- coding: utf-8 -*-
"""拉取 618 只上市股的估值因子（PE/PB/PS/PCF），带断点续跑"""
import os
import pickle
import time
import numpy as np
...
print(f"待拉取估值数据: {len(CODES)} 只", flush=True)
...
print(f"估值缓存已保存 {CACHE}: {pe_df.shape[0]}交易日 × {pe_df.shape[1]}只", flush=True)
if os.path.exists(CHECKPOINT):
